...
bool FaceMeshGraph::process(const cv::Mat *input, Landmark output[478], FaceBox &face_box) {
      ...
      if (faces.size() > 0) {
          ...
          for (int idx = 0; idx < face.landmark_size(); ++idx) {
            ...
            if (idx == 0) {
              ...
              face_box = {
                .xmin = std::min(landmark.x(), face_box.xmin),
                .xmax = std::max(landmark.x(), face_box.xmax),
                .ymin = std::min(landmark.y(), face_box.ymin),
                .ymax = std::max(landmark.y(), face_box.ymax),
                .zmin = std::min(landmark.z(), face_box.zmin),
                .zmax = std::max(landmark.z(), face_box.zmax),
              };
            }
          }
      }
